...
for($i=0;$i<count($cart);$i++){
	...
	echo '<td><div class="controls"><input class="input-small" name="quantity" type="text"/></div></td>';
	echo '<td>total</td>';
	echo '<td><center><input type="button" value="Delete" onClick="removeItem()" class="btn btn-medium btn-danger"></center></td>';
	echo '</tr>';
	}		
}

function quantity($index,$newquantity){
	$_SESSION['cart'][$index][2]=$newquantity;
}	
				...
				function addItem($order=array()){
					...
					if($_SESSION['view']>1){
						...
						if(is_array($_SESSION['cart'])){
							...
							echo "<div class='alert alert-info'><strong>Heads up!</strong><a class='close' data-dismiss='alert' href='cart.php'>&times;</a> ".count($_SESSION['cart'])." item/s added in your cart</div>";
						}
						else{
							$_SESSION['cart']=array();
							array_push($_SESSION['cart'], $order);
